...
### Vista de carga y estado de fuentes

La opción **Cargar Archivos** abre el panel de estado con los seis maestros compartidos y una matriz por local activo y Bodega Principal. Las fechas son las últimas actualizaciones publicadas, no la fecha del último movimiento. Las columnas, en orden, son ventas, detalle de pagos, MercadoPago, marketing, colaboradores, compras, tomas, transformaciones y transferencias.

**Vista Carga Anterior** mantiene los formularios, vistas previas y descargas existentes; tiene un botón para regresar al panel. **Actualizar Archivos vía API** coordina maestros compartidos, ventas/pagos, compras y fuentes originales de inventario, con avance y errores por etapa. No modifica configuraciones ni inicia descargas de MercadoPago, marketing o colaboradores, que permanecen como cargas manuales. Las fuentes de Bodega Principal se actualizan mediante La Concepción, sin duplicar consultas.

El panel obtiene el intervalo real de ventas y compras de Configuración y señala si está desactivado. Maestros e inventario permiten elegir por separado Manual, 15 minutos, 1 hora o 24 horas en la nueva vista. Guardar frecuencias persiste la configuración; el servidor encendido ejecuta las tareas aunque la pantalla esté cerrada, sin solaparlas. La primera ejecución se realiza al cumplirse el intervalo elegido y los errores se reintentan en el siguiente ciclo. El sondeo del estado de la pantalla cada cinco segundos consulta exclusivamente los datos locales. Los endpoints `/api/uploads/overview` y `/api/uploads/refresh` no exponen credenciales ni rutas internas.


La matriz permite cargar MercadoPago, Marketing y Colaboradores con **Cargar archivo** en cada celda del local correspondiente. Reutiliza inspección de estructura, confirmación de fechas y control de duplicados; no hace falta abrir Vista Carga Anterior. Las frecuencias nativas se guardan en `.integrations/toteat-api/native-refresh.json` mediante `/api/uploads/schedule`. La Bodega Principal comparte la lectura de La Concepción. El refresco nativo requiere una sesión Toteat válida y conserva los datos anteriores si falla. Los valores iniciales son Manual hasta guardar una frecuencia.

### Alcance y seguimiento de Actualizar Archivos vía API

Este botón no descarga reportes Excel/CSV de Toteat. Ventas/detalle de pagos y compras utilizan la API pública autenticada con token. Maestros (incluidas jerarquías y recetas) e inventario consultan servicios internos y datos de la aplicación autenticada de Toteat: requieren una sesión web válida, no funcionan exclusivamente con el token público. Los Excel locales que se generan son adaptaciones para los lectores internos de Brewit.

El proceso publica desde el inicio el plan completo de grupos y fuentes, conexión utilizada, estado, resultado y horas. Se muestran pendientes, actualizando, actualizados, omitidos y con problemas; un error no oculta los resultados de los demás grupos. Los grupos se marcan actualizados cuando su sincronización termina y publica sus fuentes. El último reporte se conserva en `.integrations/toteat-api/last-refresh.json`; un reinicio durante la ejecución señala las etapas incompletas como interrumpidas. MercadoPago, marketing y colaboradores quedan explícitamente fuera de la actualización automática. El estado también identifica actualizaciones independientes que mantienen ocupado el botón.
