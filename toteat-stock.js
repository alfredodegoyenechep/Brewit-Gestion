...
function createStockSync({uploadsRoot,activeLocation,credentials,reader,masters,publicReader}){
  ...
  function publish(id,source){const key=locationKey(id),config=credentials()[key];if(!config||source.restaurantId!==config.restaurantId||source.localId!==config.localId)throw Error('La fuente no corresponde al local.');
    ...
    const sales=dependency('sales',key),purchases=dependency('purchases',key),state=source.kind==='public-inventory'?require('./toteat-public-inventory').buildPublicInventory(source):buildLedger(source,products,sales,purchases);
  ...
  function synchronize(id,range){const key=locationKey(id);if(jobs.has(key))throw Error('Ya hay una actualización en curso.');const from=day(range.from),to=day(range.to);if(from>to||Date.parse(to)-Date.parse(from)>366*86400000)throw Error('Selecciona un período de hasta un año.');const c=credentials()[key];if(!c)throw Error('Configura la conexión Toteat de este local.');errors.delete(key);const job=Promise.resolve().then(async()=>{try{const source=publicReader?await publicReader(c,{from,to},current(key)?.warehouses||[]):await reader({restaurantId:c.restaurantId,localId:c.localId},{from,to,includeOperations:true});if(publicReader)atomicJson(path.join(root,key,'last-api-read.json'),source);return publish(key,source);}catch(e){errors.set(key,'No se pudo completar la actualización. Revisa los permisos de inventario y las credenciales API; se conserva la versión anterior.');throw e;}finally{jobs.delete(key);}});jobs.set(key,job);return job;}
  function view(id) {
