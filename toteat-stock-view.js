(() => {
  ...
  async function show(){const selected=location.value;try{const result=await request('/current');if(selected!==location.value)return;data=result;el('stock-warehouse').replaceChildren(new Option('Todas',''),...data.warehouses.map(w=>new Option(w.name,w.id)));
    el('stock-summary').textContent=`${data.sourceKind==='public-inventory'?'API pública Toteat':'Borrador'} · ${data.range.from} a ${data.range.to}. ${data.comparison ? `${data.comparison.comparedCells} celdas comparadas; ${data.comparison.differences.length} diferencias.` : 'Fuente sincronizada; no se utilizan archivos Kardex descargados.'} ${data.issues.length} incidencias. Los saldos con consumos pendientes no son definitivos.`;
    const grouped=new Map();for(const issue of data.issues){const key=issue.message;grouped.set(key,(grouped.get(key)||0)+1);}
    el('stock-issues').replaceChildren(...[...data.assumptions,...[...grouped].slice(0,12).map(([text,count])=>`${text}${count>1?` (${count} casos)`:''}`),data.sourceKind==='public-inventory'?'El Excel contiene agregados diarios de API, no documentos individuales.':'El Excel incluye cada documento y línea pendiente.'].map(text=>{const li=document.createElement('li');li.textContent=text;return li;}));el('stock-result').open=true;render();
