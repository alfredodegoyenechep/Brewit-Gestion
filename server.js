...
const { createToteatAutomation, TOTEAT_REPORT_URL, TOTEAT_LOGIN_URL } = require('./legacy/toteat-automation')({ ensureDir, writeJsonAtomic, readJson, cellDate });
...
function createApp(options = {}) {
  ...
  const legacyToolsEnabled = options.enableLegacyTools === true;
  ...
  legacy.register(legacyToolsEnabled, () => {
  ...
  app.post('/api/uploads/transactions/inspect', (req, res) => {
    ...
    const upload = multer({ storage, fileFilter, limits: { fileSize: MAX_FILE_SIZE, files: WEEK_FIELDS.length } }).fields(legacyToolsEnabled ? WEEK_FIELDS : WEEK_FIELDS.filter(f => legacy.manualFields.has(f.name)));
  ...
  app.delete('/api/transactions/:location/:field', (req, res) => {
    if (!legacyToolsEnabled && !legacy.manualFields.has(req.params.field)) return res.status(404).json({ error: 'Ruta histórica deshabilitada.' });
  ...
  if (legacyToolsEnabled) app.post('/api/uploads/weekly/inspect', (req, res) => {
  ...
  if (legacyToolsEnabled) app.post('/api/uploads/weekly/confirm', (req, res) => {
  ...
  if (legacyToolsEnabled) app.delete('/api/weeks/:week/:location/:field', (req, res) => {
    const { week, location, field } = req.params;
    if (!isValidWeekKey(week)) return res.status(400).json({ error: 'Invalid week.' });
    if (!activeLocation(location)) return res.status(400).json({ error: 'Select a valid location.' });
    try {
  ...
  if (legacyToolsEnabled) app.post('/upload/master', (req, res) => {
  ...
  if (legacyToolsEnabled) app.delete('/api/masters/:version/:field', (req, res) => {
    const indexPath = path.join(mastersRoot, 'masters.json');
    try {
      const index = readJson(indexPath, {});
      const group = index[req.params.version];
