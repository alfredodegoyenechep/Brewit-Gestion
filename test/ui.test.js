...
test('Análisis de la demanda abre sus cuatro capas y conserva el detalle aun sin ventas', { skip: !fs.existsSync(CHROME_PATH) }, async t => {
  ...
  const server = createApp({ uploadsRoot, reportToday: '2026-09-17' }).listen(0, '127.0.0.1');
...
test('Análisis de la demanda muestra un error visible si el servidor aún no sirve su módulo', { skip: !fs.existsSync(CHROME_PATH) }, async t => {
  ...
  const server = createApp({ uploadsRoot }).listen(0, '127.0.0.1');
  await new Promise((resolve, reject) => { server.once('listening', resolve); server.once('error', reject); });
  const browser = await chromium.launch({ executablePath: CHROME_PATH, headless: true });
  t.after(async () => {
    await browser.close();
...
test('transactional downloads show each local and report progressing in order', { skip: !fs.existsSync(CHROME_PATH) }, async t => {
  ...
  assert.equal(await purchasesRange.locator('[name="dateFrom"]').evaluate(input => input.classList.contains('changed-from-suggested')), false);
  await purchasesRange.locator('[name="dateFrom"]').fill('2026-09-01');
  assert.equal(await purchasesRange.locator('[name="dateFrom"]').evaluate(input => input.classList.contains('changed-from-suggested')), true);
  assert.equal(await purchasesRange.locator('[name="dateFrom"]').evaluate(input => getComputedStyle(input).color), 'rgb(131, 190, 255)');
  await purchasesRange.locator('[name="dateFrom"]').fill('2026-05-18');
  assert.equal(await purchasesRange.locator('[name="dateFrom"]').evaluate(input => input.classList.contains('changed-from-suggested')), false);
...
test('Cargar Archivos opens the upload workspace', { skip: !fs.existsSync(CHROME_PATH) }, async t => {
  ...
  assert.equal(await augustRent.inputValue(), '1.234.567');
  page.once('dialog', dialog => dialog.accept());
  await page.locator('.financial-general-expenses-table thead th', { hasText: 'Sept' }).dblclick();
  ...
  assert.match(await page.locator('#transaction-audit-summary').innerText(), /Sin comparación confiable/i);
  assert.match(await page.locator('#transaction-audit-body .transaction-audit-reconciliation').innerText(), /Sin archivo de pagos/);
  assert.equal(await page.locator('#transaction-audit-reconciliation-note').isVisible(), true);
  await page.locator('#transaction-audit-sources summary').click();
  assert.match(await page.locator('#transaction-audit-sources-note').innerText(), /fecha de transacción\/liquidación/);
  assert.ok(await page.locator('#transaction-audit-sources-body tr').count() >= 1);
  ...
  assert.match(await page.locator('#network-sales-metric-headings').innerText(), /Ticket con IVA/);
  assert.match(await page.locator('#network-sales-coverage').innerText(), /Costo disponible en/);
  ...
  assert.equal(await page.locator('.projection-package-input').evaluate(input => getComputedStyle(input).width), '76px');
  await page.locator('.projection-package-input').fill('1.36');
  await projectionRow.locator('td').nth(16).filter({ hasText: /^5$/ }).waitFor();
  assert.equal(await projectionRow.locator('td').nth(12).textContent(), '4,1 UN');
  await page.locator('.projection-package-input').fill('1');
  await projectionRow.locator('td').nth(16).filter({ hasText: /^4$/ }).waitFor();
  ...
  await page.evaluate(() => {
    const base = purchaseProjectionState.items[0];
    purchaseProjectionState.items.push({
      ...base,
      key: 'P2',
      code: 'P2',
      name: 'Ingrediente adicional del proveedor',
      managed: false,
      needsPurchase: false,
      suggestedPurchaseUnits: 0,
      suggestedInternalQuantity: 0,
      estimatedTotal: 0
    });
  ...
  assert.match(await page.locator('#purchase-order-editor-item-count').textContent(), /1 visibles · 2 del proveedor/);
  await page.locator('#purchase-order-show-all-supplier-items').check();
  assert.equal(await page.locator('#purchase-order-editor-body tr[data-key]').count(), 2);
  const additionalOrderRow = page.locator('#purchase-order-editor-body tr[data-key="P2"]');
  await additionalOrderRow.locator('.purchase-order-select').check();
  assert.equal(await additionalOrderRow.locator('.purchase-order-quantity').inputValue(), '1');
  assert.match(await additionalOrderRow.locator('.purchase-order-row-warning').textContent(), /Ítem adicional/);
  await additionalOrderRow.locator('.purchase-order-select').uncheck();
  await page.locator('#purchase-order-show-all-supplier-items').uncheck();
  assert.equal(await page.locator('#purchase-order-editor-body tr[data-key]').count(), 1);
  ...
  await page.evaluate(() => {
    const savedItem = purchaseOrderEditorState.items.find(item => item.selected);
    purchaseOrderEditorState.items.push({
      ...savedItem,
      key: 'ZERO',
      code: 'ZERO',
      name: 'Ítem con cantidad cero',
      quantity: 0,
      internalQuantity: 0,
      total: 0,
      selected: true
  ...
  assert.equal(await page.locator('#purchase-order-document tbody tr').count(), 1);
  assert.doesNotMatch(await page.locator('#purchase-order-document').textContent(), /Ingrediente adicional|cantidad cero/i);
  assert.match(await page.locator('#purchase-order-document tfoot').textContent(), /TOTAL ORDEN.*\$2\.000/s);
  ...
  await page.locator('#projection-only-required').check();
  assert.equal(await page.locator('#purchase-projection-body tr[data-key]').count(), 0);
  await page.locator('#projection-only-required').uncheck();
  ...
  const inventorySectionOrder = await page.locator('#inventory-report-results').evaluate(section => ({
    ...
    consumption: section.querySelector('#inventory-consumption-reports').getBoundingClientRect().top,
    kardex: section.querySelector('.inventory-kardex-heading').getBoundingClientRect().top
  }));
  assert.ok(inventorySectionOrder.executive < inventorySectionOrder.consumption);
  ...
  assert.equal(consolidatedWorkbook.SheetNames.includes('Resumen ejecutivo'), true);
  ...
  await nameInput.locator('xpath=..').getByRole('button', { name: /Guardar cambios/ }).click();
  ...
  await page.getByRole('link', { name: 'Costos por Revisar' }).click();
  await page.locator('#cost-review-status').filter({ hasText: /Costos al .*cafeterías/i }).waitFor();
  assert.equal(await page.locator('#cost-review-date').inputValue(), '2026-08-10');
  assert.ok(await page.locator('#cost-review-body tr').count() > 0);
